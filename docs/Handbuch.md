...
| 0.54 | 25.07.2026 | Performance-Optimierung, kein Verhaltensunterschied bei einem der 34 Prüfpunkte (gezielter Review auf User-Auftrag, vollständige Analyse siehe `XML-Optimierung-Analysebericht.md` im Vault-Root): Bestandsaufnahme ergab, dass `export_block_xml()` (Baustein-XML-Export für Netzwerk-/Interface-Analyse, `_tia_helpers.py`) von bis zu 10 unabhängigen Stellen in `comments.py`/`structure.py`/`libraries.py` ohne jeden Cache aufgerufen wurde — ein typischer FB wurde dadurch pro Lint-Lauf bis zu 8-mal exportiert (Prüfpunkte 1c, 3, 10, 11a, 14, 15, 16, 26), zusätzlich einmal je Instanz-DB über Prüfpunkt 25. Hochrechnung anhand der dokumentierten Bausteinzahlen des Maschine-Projekts (288 Bausteine, davon 127 FB, restliche Aufteilung geschätzt): ca. 1.777 → 237 `Export()`-Aufrufe, Faktor ≈7,5 (~87 % weniger). Zweiter, unabhängig gefundener Fall: `ProjectTextComments.load()` (`project_texts.py`, projektweiter Excel-Export aller Projekttexte für DB-Variablen-Kommentare) wurde von den Prüfpunkten 1a/1b/1c jeweils pro PLC-Software erneut aufgerufen, statt das Ergebnis wiederzuverwenden. Fix: Beide Exporte cachen ihr Ergebnis jetzt lauf-gebunden (`export_block_xml()`/neue Funktion `reset_export_cache()`; `ProjectTextComments.load()`/neue Methode `.reset_cache()`). Der Cache-Key ist bewusst ein stabiler String (`plc_software.Name` + `block.Name`), keine Objektreferenz — `runner.py` verbindet die TIA-Portal-Session bei größeren Projekten periodisch neu (siehe Abschnitt 4.2), wonach alle vorherigen .NET-Objekte disposed sind. `runner.py` leert daher beide Caches explizit bei jedem (Re-)Connect (`_reset_run_caches()`); ein zwischenzeitlich fehlgeschlagener Export wird dadurch beim nächsten Zugriff erneut versucht statt dauerhaft als fehlgeschlagen zu gelten — fehlgeschlagene Exporte werden aus demselben Grund grundsätzlich nie gecacht. Alle 10 Aufrufstellen von `export_block_xml()` auf die neue Signatur `export_block_xml(block, plc_software)` umgestellt, Prüflogik und Rückgabewerte unverändert. Neuer Test `TestExportBlockXmlCache` (`tests/test_tia_helpers.py`, 5 Fälle: Cache-Hit bei gleichem Baustein, unabhängige Bausteine, gleicher Blockname auf zwei PLCs, Reset erzwingt Re-Export, fehlgeschlagener Export wird nicht gecacht) — reine Mock-Verifikation ohne TIA Portal/pythonnet, da auf Linux entwickelt. `pytest` 51/51 → 56/56 grün. Abschnitt 4.2 um einen Hinweis auf den Cache-Reset bei Reconnect ergänzt. |
