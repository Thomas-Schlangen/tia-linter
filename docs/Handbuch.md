...
**Version dieses Handbuchs:** 0.46 (Entwurf)
**Stand:** 23.07.2026
...
| Parameter | Standardwert | Bedeutung |
|---|---|---|
| `ignorierte_meldungen` | `[]` | Liste von Regex-Mustern; ein Treffer irgendwo im Meldungstext unterdrückt die betreffende Compiler-Meldung vollständig und dauerhaft. |
...
- Für das Übersetzen müssen laut TIA-Portal-Vorgabe alle Geräte offline
  ...
  …").
- `ignorierte_meldungen` prüft — anders als sonst bei Regex-Parametern in
  diesem Programm üblich — per `re.search`, nicht `re.match`: ein Muster
  darf irgendwo im oft langen, freien Compiler-Meldungstext treffen, nicht
  nur an dessen Anfang. Gedacht für Meldungen, die bewusst und dauerhaft
  als irrelevant eingestuft werden, typischerweise weil sie bei sehr vielen
  schreibgeschützten Bausteinen auftreten (z. B. aus zugekauften/bereits
  fertigen Bibliotheken) — Beispiele aus der Praxis: `"since it is
  write-protected"`, `"because it is not editable"`. Ein Treffer
  unterdrückt die Meldung unabhängig von Baustein und Fehler-/
